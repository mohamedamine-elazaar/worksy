...
/**
 * Signup Page
 * Handles user registration for different roles (freelancer, intern, company).
 * Uses authApi to communicate with the backend.
 */
export default function Signup() {
  const { t } = useTranslation()
  const navigate = useNavigate()
  const [role, setRole] = useState("freelancer") // freelancer | stagiaire | entreprise
  const [values, setValues] = useState({ name: "", email: "", password: "", company: "" })
  ...
  const onSubmit = async (e) => {
    ...
    let mappedRole = role;
    if (role === 'stagiaire') mappedRole = 'stager';
    // 'freelancer' and 'entreprise' match the backend enum directly.
